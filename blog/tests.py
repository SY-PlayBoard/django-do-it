...
class TestView(TestCase):
    ...
    def test_post_detail(self):
        ...
        post_001=Post.objects.create(
            title='첫번째 포스트',
            content='디지털데일리 권하영기자] 국내 배달앱 2위 요기요 인수전이 안갯속에 빠졌다.',
        )

        #1.2 그포스트의 url은 'blog/1/'이다.
        self.assertEqual(post_001.get_absolute_url(), '/blog/1/')

        #2. 첫번째 포스트의 url은 'blog/1/'이다
        #2.1. 첫 번째 post url로 접근하면 정상적으로 작동한다.(status code:200).
        response=self.client.get(post_001.get_absolute_url())
        self.assertEqual(response.status_code, 200)
        soup=BeautifulSoup(response.content, 'html.parser')

        #2.2포스트 목록 페이지와 똑같은 내비게이션 바가 있다.
        navar=soup.nav
        self.assertIn('Blog', navar.text)
        self.assertIn('About Me', navar.text)
